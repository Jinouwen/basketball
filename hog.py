...
class Hog:
    ...
    def compute(self, img, block_norm_method = 'L2-Hys', visualize: bool = False):
        ...
        if visualize:
            ...
            canvas = np.zeros((self.window_size[0], self.window_size[1], 3), dtype=np.float)
            radius = min(self.cell_size[0], self.cell_size[1]) // 2 - 2
            ...
            for i in range(self.ncell[0]):
                for j in range(self.ncell[1]):
                    center_y = i * self.cell_size[0] + self.cell_size[0] // 2
                    center_x = j * self.cell_size[1] + self.cell_size[1] // 2
